...
class MidiMusician:
    ...
    def play_notes_even_with_lag(self, instrument, notes, duration, velocity, lag):
        startTime = self.T
        for note in notes:
            self.play_note(instrument, note, duration, velocity, True)
        self.T = startTime + lag

    def play_notes_even_with_lag_with_rando_veloc(
        self, instrument, notes, duration, velocity, lag
    ):
        startTime = self.T
        for note in notes:
            v = random.randint(0, 100)
            self.play_note(instrument, note, duration, v, True)
        self.T = startTime + lag
